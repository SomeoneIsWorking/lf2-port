...
static void lift_function(FILE *o, const Func *f)
{
    ...
    /* An overridden function still gets its body emitted, under a shadow name. The native
     * implementation in runtime/overrides.c provides fn_<addr> and may call
     * fn_<addr>__orig() for any path it has not ported yet. Without that, porting a large
     * function would be all-or-nothing: fn_004246b0 is 4689 lines covering three menu
     * screens, and there is no way to land that in one correct step. */
    if (is_overridden(f->addr)) {
        fprintf(o, "\n/* fn_%08x is overridden; original body kept as the shadow below. */\n",
                f->addr);
        fprintf(o, "void fn_%08x__orig(void)\n{\n", f->addr);
    } else {
        fprintf(o, "\nvoid fn_%08x(void)\n{\n", f->addr);
    }
...
int main(int argc, char **argv)
{
    ...
    for (int i = 0; i < nfuncs; i++) {
        fprintf(o, "void fn_%08x(void);\n", funcs[i].addr);
        if (is_overridden(funcs[i].addr))
            fprintf(o, "void fn_%08x__orig(void);\n", funcs[i].addr);
    }
