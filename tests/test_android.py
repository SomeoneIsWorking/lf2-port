...
SPEC = importlib.util.spec_from_file_location(
    "lf2_android", ROOT / "tools" / "build" / "android.py"
)
...
def expect_refused(operation: Callable[[], object], needle: str) -> None:
    try:
        ...
        raise AssertionError(
            f"operation accepted input that should be refused: {needle}"
        )
...
def main() -> int:
    ...
    assert (
        ANDROID.android_version_name({"LF2_ANDROID_VERSION_NAME": "0.1.3"}) == "0.1.3"
    )
    ...
    assert (
        ...
        in (ROOT / "platforms" / "android" / "build.gradle").read_text()
    )
    ...
    assert (
        'android:name="io.github.someoneisworking.lf2port.UpdateFileProvider"'
        in manifest
    )
    updater = (
        ROOT
    ...
    expect_refused(
        lambda: ANDROID.inspect_apk(game_apk), "prohibited original game paths"
    )
    ...
    shutil.rmtree(build)
    print(
        "android tool: signing and APK content gates passed positive and negative cases"
    )
    return 0
