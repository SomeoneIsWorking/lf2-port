...
def run(binary: Path, selection: Path, config: Path, expected_success: bool) -> subprocess.CompletedProcess[str]:
    environment = dict(os.environ)
    environment["XDG_CONFIG_HOME"] = str(config)
    result = subprocess.run(
        [str(binary), str(selection)], text=True, capture_output=True, env=environment, check=False
    )
...
def main() -> int:
    ...
    with zipfile.ZipFile(nested, "w", zipfile.ZIP_DEFLATED) as archive:
        ...
        archive.writestr("Install/LF2/data/data.txt", b"fixture")
    ...
    print("game selection: direct, directory, nested ZIP, duplicate, traversal, and budget checks passed")
    return 0
