...
#
# Only ONE join press, unlike tools/routes/coop_dropin_test.sh, and that matters: a second attack
...
( cd "$GAME" && \
  ...
  LF2_QUIT_AFTER=1960 timeout -k 5 220 "$BUILD/lf2" lf2.exe ) > "$LOG" 2>&1
