...
#
#     tools/scratch_clean.sh screenshots/42        -> empties scratch/screenshots/42
#     tools/scratch_clean.sh logs                  -> empties scratch/logs
set -eu
...
if [ "$#" -ne 1 ]; then
    echo "usage: tools/scratch_clean.sh <path under scratch/>" >&2
    exit 2
fi
