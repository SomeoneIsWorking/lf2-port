...
compares the two independent sources:
  ...
  file      tools/decrypt_dat.py over every bg.dat under game/bg, decrypted offline with the
            game's own cipher.
...
exits non-zero saying it compared NOTHING rather than reporting a vacuous pass.
    ...
    tools/bg_table_check.py <log-with-LF2_BG_TABLE=all-output> [game-dir]
    tools/bg_table_check.py --selftest
"""
import glob
import os
import re
