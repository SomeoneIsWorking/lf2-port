...
The recorded object-pass comparison masked exactly these -- the cookie as two dwords, the date strings as their +/-16 byte buffers, because which dwords of a string differ depends on how far the clock moved -- and required zero differences elsewhere. Its negative arm (a different game mode) differed by 73-78 dwords in `.data` and by the heap's size, so the comparison was shown to fire.

## What would falsify it

any pair of identical runs differing at an address outside {0044eea4, 0044eea8} and the three date-string buffers at 0044fda4 / 00451d58 / 00458360 (`.data`) and 20000040 (heap).
