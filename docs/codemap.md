...
| Drop-in coop (RE) | `runtime/overrides.c`, issue #15 | **wip** | The blocking RE question is answered and demonstrated: `this` is `0x00458b00`, `this+404` is **400** object pointers on a `0x420` stride (the eight player slots are its first eight entries, all pre-allocated and non-null at all times), and an object is in the world iff the **byte at `0x00458b04 + index`** is 1 — the test `fn_004064d0` makes as it walks the table. `LF2_COOP_SPAWN=<index>` clones a fighter into a **running** match: it animates, walks, collides, takes damage and draws with its own HUD bar, where the identical clone without the gate byte is inert for 300 frames. What remains is the feature, not the mechanism — choosing a free index, building the record from the chosen character rather than cloning a neighbour, and binding it to the joining device (the ported gather still caps at `i < 4`) |
