...
| Global RmlUi, input mapping and device art | `runtime/ui/settings_ui.cpp`, `runtime/ui/rmlui_backend.cpp`, `runtime/ui/device_assets.c`, `runtime/ui/device_icons.c`, `runtime/input/bindings.c`, `runtime/input/keyboard.c`, `runtime/input/gamepad.c`, `runtime/app/pause.c`, `runtime/app/config.c`, `tests/test_bindings.c`, `tools/routes/settings_test.sh`, `tools/routes/ui_escape_test.py`, `tools/routes/ui_global_test.sh`, issues #70 #77 #79 #80 | **done** | Escape or Start opens one Dusklight-structured RmlUi shell directly on mode menu, character selection, overlay, and match; the removed hand-painted pause menu is no longer a prerequisite. Physical key state is recorded before RmlUi consumes SDL events, and Escape is a one-shot event edge rather than a held-state poll: a complete tap drained in one pump still opens the shell, and key releases under the modal cannot stick after close. `ui_escape` focuses the real X11 window and uses XTEST—not `LF2_KEY_SCRIPT`—to prove physical Escape opens and closes it; the old scripted test was a false positive because only scripts reached the ledger. RmlUi owns presentation/navigation and its SDL backend remains separate. While active it consumes all physical input before Win32 messages and zeros the native input gather; only a match freezes, with its retained native display list rewound under the modal. Persistent mappings feed the shipping gather. Shared `port-assets` SVGs are embedded rather than copied; native icons rasterize to their current output footprint, RmlUi uses a 120x120 linear-filtered raster, and software retains logical size. |
...
| Input path | `runtime/win32/win32.c`, `runtime/input/keyboard.c`, `runtime/input/bindings.c`, `runtime/overrides/input.c`, `runtime/ui/settings_ui.cpp`, issue #81 | **done** | Keyboard, mouse and controller enter game state through their native paths; RmlUi edits the action maps consumed by those shipping paths. The Win32 pump translates SDL events, then `keyboard.c` records physical state before modal consumption and latches Escape's down edge for the app shell. Only unowned keys reach guest messages. While RmlUi is open it consumes SDL input and zeros the direct keyboard/pad gather so no action leaks underneath. |
...
| Menus: one input model | `runtime/overrides/menu.c`, `runtime/overrides/screens.c`, `runtime/ui/settings_ui.cpp`, `runtime/app/pause.c`, `tools/routes/mouse_test.sh`, `tools/routes/settings_test.sh`, `tools/routes/ui_global_test.sh` | **done** | Normal boot starts at the post-load **mode menu**. Mode selection, **character selection** (hover moves the slot cursor, click joins and picks), and the **pre-fight overlay** retain the game's own behavior. Escape/Start opens RmlUi directly on each of those screens and in a match; no legacy pause screen exists. A mouse-only route reaches a match, the settings route drives the document by mapped controller actions, and `ui_global` opens and closes the same shell in all four contexts. The retired launcher has no port-owned navigation or reachable replay/network choices. |
