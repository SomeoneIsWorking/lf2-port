...
| Native renderer | `runtime/render.c`, `runtime/render.h`, `tools/render_test.sh`, issue #30 | **wip** | The game's draws as **GPU geometry**. `ddraw.c` records a display list per destination surface; the game composes off-screen and copies to the primary, so the source of that copy *names* the composition and the frame boundary is discovered from the game's own blits with no address hardcoded. **The colour key becomes alpha** on upload — that is the port's first blend stage, the thing claim C010 said made shadows and bloom impossible. Drawn into a **render target at the window's resolution**, not the game's: the software path fills a 794x550 buffer and point-scales it, so on a 1080p screen every pass ran on an upscaled thumbnail. GDI text, which never goes through `Blt`, is carried as premultiplied **tiles** placed at their real position in the list. An **HD2D bloom** runs on the target — bright pass by `SDL_BLENDMODE_MOD` of the frame over itself (squaring is a soft threshold with no shader), then two linear downsamples as the blur, then additive. No shader toolchain is required. `LF2_RENDERER=soft` still presents the software compositor, and `ctest render` diffs them: **max 1–2 levels of 255**, all on antialiased glyph edges where the CPU blends against the destination and the GPU blends a premultiplied tile. Negative arms: dropping every 7th draw must differ, and the HD2D pass must change the frame. **Not done:** sprite-cast shadows (the game's own is a dithered checkerboard, plainly visible once magnified), depth-sorted lighting, and text rendered at output resolution rather than upscaled. The pause menu and controls hint still present through the software path because they draw straight onto the primary and are in no display list |
