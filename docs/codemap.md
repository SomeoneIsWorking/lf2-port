...
| Renderer: object-sprite sampling chain | `runtime/video/spritefilter.h`, `runtime/shaders/quad.frag`, `runtime/video/engine.c`, `runtime/app/options.c`, `runtime/ui/settings_ui.cpp`, `tests/test_spritefilter.c`, `tools/routes/sprite_passes_test.py`, issues #112 #113 | **done** | A player builds an ordered chain of resampling passes over magnified object sprites, plus edge reconstruction and a silhouette outline, from the GRAPHICS tab: `nearest`/`linear` passes with rational or AUTO factors, `aa`, `outline:1..2`. `spritefilter.h` owns parsing, AUTO, draw isolation, edge-wedge geometry and outline extent; `quad.frag` evaluates the chain directly in the drawable-sized scene target. `aa` is an original alpha-aware perceptual 3x3 classifier: it groups shaded contour pixels, requires outside/inside continuation, extends the reconstructed cut for shallow or steep runs, and returns the centre texel exactly outside classified wedges. Source origin and extent travel as explicit sheet-texel values because texel-centre UV endpoints span extent-1 and cannot be inverted reliably. There is no sprite prepass or lower-resolution intermediate. `tools/e2e.py sprite_passes` uses one authenticated 3840x1975 match frame: the engine target and captures must be 4K, `nearest:2` must be byte-identical to no chain, `nearest:1/2` and `outline:1` must change the frame, and isolated `aa` must change real fighters while changing zero pixels in flat 5x5 output regions. The measured acceptance frame changed 3,920 edge-local pixels, 0 flat-interior pixels; coarsening changed 22,621 and the outline changed 7,092. `ctest spritefilter` walks the parser, caps, AUTO factor, outline margin, coordinate chain and diagonal/shallow/steep edge coverage. |
