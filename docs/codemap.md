...
| Guest memory and PE image | Map the authenticated PE and guest arenas; expose checked reads/writes and executable-write notification | `runtime/cpu/` | `guest.c` (PE loading), `guest_memory.{c,h}` (backing and checked native spans), `guest_arena.{c,h}` (bounded allocation), `guest_map.h` (address ownership) | `docs/migration.md` |
...
| Win32 and CRT HLE | Window/message, CRT/import, case-insensitive path, GDI, DirectShow, and socket services | `runtime/win32/` | `imports.c`, `win32.c`, `paths.c`, `gdi.c` (handles/painting), `gdi_bitmap.c` (bounded bitmap files/resources), `dshow.c`, `wsock.c` | `docs/platform-boundary.md` |
| Guest allocation policy | Preserve each subsystem's addresses, alignment, bounds and session lifetime | `runtime/win32/`, `runtime/video/`, `runtime/audio/` | `guest_heap.c`, `com.c`, `surface_memory.c`, `dsound.c` consume `guest_arena.h` | `docs/migration.md` |
...
| Host UI | RmlUi document/backends, the first-run setup screen and what it asks for, device art, and touch presentation; edits configuration but does not own it. The setup screen itself is `shared/setup-ui`; `setup_screen.cpp` drives it and `setup_screen_policy.cpp` says what LF2 asks for and how it judges the answer | `runtime/ui/` | `settings_ui.cpp`, `setup_screen.cpp`, `setup_screen_policy.cpp`, `touch_controls.c` | `AGENTS.md` |
| Game-file picker | Asking the host for the location holding the player's LF2 files: SDL's chooser on desktop, the Activity's SAF hand-off on Android. Decides nothing about what the location contains | `runtime/platform/` | `game_picker.c` | `docs/running.md` |
| Platform integration | Android title JNI/window policy and setup wording; shared/android-port owns Activity lifetime, SAF bounded staging and contained-directory publication. `Lf2StageAssets` owns bundled title stage extraction | `runtime/platform/`, `platforms/android/` | `android_bridge.c`, `window_policy.c`, `Lf2Activity.java` | `docs/running.md` |
...
| Packaging and release publication | AppImage, macOS app, and title Android assembly/signing/updater/content policy; validate a single tag/version and exact package set before GitHub publication. Android profiles consume shared/android-port native dependencies, runtime staging, JDK selection and APK runtime checks | `tools/build/`, `platforms/`, `.github/workflows/release.yml`, `CMakeLists.txt` | `appimage.py`, `macos.py`, `android.py`, `release_context.py`, `release_manifest.py` | `docs/running.md` |
| Browser gesture reachability | Give the game canvas the browser's own scroll, pinch, callout and overscroll gestures, through the shared owner rather than a local declaration | `web/app.mjs`, `web/style.css`, shared `web-port` `platforms/web/canvas.mjs` | `claimCanvasGestures` | `docs/project-state.md` |
| Browser package | Build the asset-free native/JIT WASM product with shared web-port storage and isolation, and upload its CI artifact; sibling `pages` owns public deployment | `tools/build_web.py`, `.github/workflows/browser-package.yml`, `web/`, `src/web/`, shared `web-port` runtime | `build_web.py` | `docs/project-state.md` |
