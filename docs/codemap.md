...
| Project structure | `AGENTS.md`, `CLAUDE.md`, `tools/build/check_structure.py`, global `game-port-structure` skill | **done** | Host ownership follows Dusklight's pattern rather than its platform implementation: `app` composes lifecycle/startup, `ui` separates RmlUi document from SDL backend, `input` owns mappings, `video` owns rendering, and `overrides` owns guest behavior. The normal ctest suite caps new runtime sources at 500 lines and freezes oversized legacy sources at their current count. |
| Direct startup | `runtime/app/startup.c`, `runtime/overrides/boot_guest.c`, `runtime/video/ddraw.c`, `runtime/overrides/menu.c`, issue #71 | **done** | The game enters the original Game Start branch's non-rendering state directly after platform initialization: front-end sound stops, local-play marker and device slots are initialized, and top mode becomes loader. No key, click, pointer, or controller input is manufactured. The real loader runs, but presentation is suppressed until mode 2; the first visible/script-detectable screen is `modemenu` at frame 4. Smoke verifies that landing and then reaches a match. The retired front end's keyboard/controller adapters are deleted and its network/recording paths are absent from normal boot; `wsock.c` remains a dormant stub, not a network design for the port. |
| RmlUi settings, input mapping and device art | `runtime/ui/settings_ui.cpp`, `runtime/ui/rmlui_backend.cpp`, `runtime/ui/device_assets.c`, `runtime/ui/device_icons.c`, `runtime/input/bindings.c`, `runtime/input/gamepad.c`, `runtime/app/config.c`, `tests/test_bindings.c`, `tools/routes/settings_test.sh`, issues #70 #77 | **done** | RmlUi owns the settings document; the SDL render backend is a separate adapter. Keyboard and controller bindings for Up/Down/Left/Right/Attack/Jump/Defend are persistent and consumed by the shipping input gather. Controller capture waits for release so the button that opens capture cannot bind itself. The settings headers, character-select slots, and in-match HUD use keyboard/gamepad SVGs from the shared `port-assets` checkout. One C asset module resolves/rasterizes the build-embedded SVGs for RmlUi, the software compositor, and GPU display-list tiles; there is no LF2-local icon copy or runtime host path. The `settings` route proves the document opens, renders, and loads both shared textures. |
| Renderer: engine and stable composition | `runtime/video/engine.c`, `runtime/video/engine.h`, `runtime/video/render.c`, `runtime/shaders/quad.vert`, `runtime/shaders/quad.frag`, `tools/routes/render_test.sh`, issues #64 #76 | **done** | One SDL_GPU device, one texture pool, painter-order D32 depth, three blend pipelines. The display list is unchanged; the engine replaces what draws it. The whole world is now composed on the native integer grid and scaled once as a finished texture. Previously every layer was independently placed at fractional output coordinates, changing its nearest-sampling phase during camera motion and producing background jitter. Native-grid composition gives all layers one stable phase. The software renderer remains the byte-identity A/B control. |
...
| Input path | `runtime/win32/win32.c`, `runtime/input/bindings.c`, `runtime/ui/settings_ui.cpp` | **done** | Keyboard, mouse and controller enter game state through their native paths; RmlUi edits the keyboard/controller action maps consumed by those shipping paths. |
...
| Legacy netplay | `runtime/win32/wsock.c` | **intentionally dormant** | The skipped front end makes the game's network path unreachable in normal boot. The WinSock boundary remains a loud stub for binary compatibility only; any future multiplayer is a separate custom rollback subsystem, not an implementation of the game's protocol. |
...
| Menus: one input model | `runtime/overrides/menu.c`, `runtime/overrides/screens.c`, `runtime/ui/settings_ui.cpp`, `tools/routes/mouse_test.sh`, `tools/routes/settings_test.sh` | **done** | Normal boot starts at the post-load **mode menu**. From there mode selection, **character selection** (hover moves the slot cursor, click joins and picks), the **pre-fight overlay** (`0x0044d06c`, phase word `0x0044d070`), pause, and RmlUi settings accept the devices each screen supports. A mouse-only route reaches a match with no key and no pad; the settings route opens RmlUi by pad. The retired launcher has no port-owned keyboard/controller navigation. |
