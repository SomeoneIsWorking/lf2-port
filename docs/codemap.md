...
| AppImage release | `CMakeLists.txt` install rules, `platforms/freedesktop/`, `tools/build/appimage.py`, `tools/build/release_dependencies.py`, `tests/test_appimage.py`, issues #115/#117 | **done** | The CMake install tree is the only packaging input. Pinned linuxdeploy/appimagetool/runtime hashes create an x86_64 artifact; both the staged tree and extracted final image are scanned to refuse original PE/game-data paths. Maintainers build and gate the ignored artifact locally in Ubuntu 22.04, then upload it to the GitHub Release manually; no hosted workflow or generated package/source output is committed. The port-owned translated executable ships; the installer, original `lf2.exe`, and extracted assets do not. |
