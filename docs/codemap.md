...
| Renderer: the engine | `runtime/video/engine.c`, `runtime/video/engine.h`, `runtime/shaders/quad.vert`, `runtime/shaders/quad.frag`, `tools/routes/render_test.sh`, issue #64 | **wip** | **The port's own renderer.** One SDL_GPU device (the one SDL's `gpu` renderer is already built on, C029), one D32_FLOAT depth buffer, one texture pool, three blend pipelines. It exists because SDL_Render is a 2D sprite API and this port draws a lit scene with pixel art in it — no depth attachment, so geometry needed a second renderer; the two cannot share a texture (C032, falsified by measurement), so art was on the GPU twice; they could only meet as a texture, so a set cost a render target per parallax gap; and the key light had to be duplicated and drifted 15°. **Three pipelines, not one**, because SDL_GPU fixes blend at pipeline creation — premultiplying everything on upload instead would change what a keyed sprite's colour *is*, and the byte-identity arms compare exact pixels. **Depth is the list ordinal**: later is nearer, test `LESS_OR_EQUAL`, so every quad passes and the picture is exactly the painter order's while the depth buffer holds a real value per pixel. The painter order is the GAME's answer (it sorts sprites on z; the object/shadow pairing was learned from that order, C019) and is not second-guessed. **The display list is untouched** — this replaces what *draws* it, not what records it. **Verified** (C034) against the software compositor at max channel diff 1 (251 px) and 2 (386 px), the same tolerance the old GPU path gives, with its own dropped-draw arm proving the engine is what drew the matching frame. `LF2_ENGINE=1` selects it; the SDL_Render path stays as the A/B control arm, as `LF2_BG_ORIG` does for the background override. **Known:** batching is ~1.1 quads/batch — only *consecutive* quads sharing a texture may merge, because reordering would change the order the depth comes from. **A G-BUFFER carries what effects need** (issue #63): a second colour attachment, `R16G16B16A16_FLOAT`, holding a surface normal and the draw's real **DISTANCE** as a parallax depth. Distance is *not* the depth buffer's depth — that one is the painter order, which decides what covers what, and a defocus driven off it would be blur by draw order. Layers get `(stage_width-794)/(span-794)` (C031), geometry its own per-vertex depth, sprites nothing: they write a normal of exactly `(0,0,0)`, never a unit vector, which is an exact marker for "no surface here". A colour attachment rather than the depth buffer because **SDL3 declares no pixel format for depth at all** (checked in `SDL_pixels.h`), so a D32_FLOAT texture cannot be wrapped as an `SDL_Texture`; 16-bit float because the distance runs 0.89–535 on the shipped stages and 8 bits of that is a staircase. Never blended — halfway between a sprite at 1.0 and a sky at 535 is a plane nothing occupies. **Read back and checked against the stage's own `bg.dat`** (`LF2_ENGINE_GBUF=1`, I016): Brokeback Clif reports 1.0000 and 1.2061, which is C031's 706/585 at half-float precision, with 90 px carrying a real normal when a `.stage` is authored and 0 px when none is. The readback pulls the **colour target back beside it**, so it can report the CONJUNCTION an effect is actually driven by -- how many pixels are both bright and in the world -- plus the world's own luminance histogram. That pairing is what killed the luminance bloom (C035): on Brokeback Clif 766 px sit above 0.75 luminance and **not one of them carries a distance**, so every bright pixel in the frame is HUD, text or a sprite. Two histograms read separately would each have looked fine. **Not in it yet:** the lighting (see below). |
| Renderer: depth of field | `runtime/shaders/dof.frag`, `runtime/video/engine.c`, `tools/routes/render_test.sh`, issue #63 | **done** | **Defocus by DISTANCE**, off the engine's G-buffer (see the engine row) — the effect that was deleted once for reading as a filter over a screenshot, and the one that earned its way back because distance is a measured quantity. It is a function of the G-buffer's distance and nothing else: focus at **1.0**, the fighters' plane, by derivation rather than taste (a parallax depth of 1 is where an object shifts with the camera at rate 1, which is where the game puts every fighter). The measure is **1/d, not d** — defocus is a difference of reciprocals, and the reciprocal maps the shipped stages' 0.89–535 into (0, 1.1] instead of cramming them into the last few percent. A tap only counts if it is at the **same distance**, compared as a *ratio* because distance is a scale — without it a blurred sky pulls a sharp fighter's colour across its own silhouette, the classic halo. Radius is in output texels (`oh/110`) so a blur is a function of distance and not of resolution. **Fighters are never blurred, and not as a special case**: they write no distance, so they take the same untouched branch the HUD does. Asserted opposite ways on two frames — it changes **nothing** (max diff 0) on a frame with no stage in it and **84,395 px** in a match — and the first holds by construction, which is what catches an effect spreading over the whole picture. `LF2_DOF=off` is the control arm. **Known:** it runs before `hd2d`'s lighting rather than after, so a cast shadow stays sharp on a blurred surface; invisible so far because the floor a shadow falls on is at depth 1.0 and in focus. |
| Renderer: display list | `runtime/video/render.c`, `runtime/video/render.h`, `runtime/video/texrect.h`, `tools/routes/resize_test.sh` | **done** | The game's blits, recorded per destination surface as an ordered scene — the part that came out of measurement and is right. The game composes off-screen and copies to the primary, so the **source of that copy names the composition** and the frame boundary is discovered from the game's own blits with no address hardcoded. **The colour key becomes alpha** on upload, which is where this port's blend stage comes from at all (it is what claim C010 said made shadows impossible). **Scaled per quad** (issue #41): the window sets a world scale of `min(win_h/550, win_w/794)`, so height buys magnification and leftover width buys FIELD OF VIEW; each quad is scaled as it is drawn into a full-resolution target, so a sprite is resampled exactly once, from the source art. Height cannot buy field of view — LF2's vertical axis carries z and jump height, both fixed by stage data. At 794x550 the scale is exactly 1 and every byte-identity arm holds. Textures are cached, but every DirectDraw/GDI surface write explicitly invalidates both GPU caches; the sampled content hash is only a backstop because sampling can miss a one-row change. A strict sprite-sheet subrect addresses the **centres** of its first and last texels (`texrect.h`, issues #67/#68), never the boundary shared with the next animation cell; whole-surface pictures retain 0..1 UVs. GDI text rides as premultiplied **tiles**; the pool is keyed on a 32-px bucket, because 'a tile's size repeats constantly' is true of a glyph cell and false of `TextOutA`, whose tile is a whole string. **The port's own UI is in the list** (issue #52) and marked, so it is drawn *after* the lighting — before it, the character mask re-composited a fighter on top of the pause panel. A frame's lists are cleared by the first call that RECORDS over them, not by the present that finished it, because pause freezes the game by declining to update and no blit arrives. |
| Renderer: HD2D lighting | `runtime/video/hd2d.c`, `runtime/video/hd2d.h`, `runtime/video/stagelight.h`, `tests/test_stagelight.c`, `runtime/shaders/hd2d_gbuf.frag`, `runtime/shaders/hd2d_light.frag`, `runtime/shaders/hd2d_shadow.frag`, `tools/build/build_shaders.sh`, issues #30 #37 | **wip** | One key light as a **direction in the stage's own axes** (x across, y up = LF2's jump axis C018, z toward the camera), hemisphere ambient, and a bevel normal from the gradient of the sprite's silhouette — applied **only to objects standing in the stage**, which the game identifies for free by drawing a shadow ellipse at their feet immediately before them (C019). HUD and text are untouched. The cast shadow is that sprite's silhouette sheared along the **same** vector, crisp (a blurred half-res mask of a 32-px sprite is a smear), and needs its own shader because SDL's fixed-function multiply gives `sprite.rgb * a` — the fighter's colours — which made a shadow darker under the bright parts of them. **The floor is lit as a floor:** `bg.dat`'s `zboundary:` is where the walkable floor begins on screen (C021, 12 of 12 stages), applied as colour only at locked luminance, gated on the in-match HUD. **The light is ONE vector, in a pure header** (`stagelight.h`, `ctest stagelight`, I015) — `mesh.c` once held a copy 15° away under a comment saying they matched, and `hd2d`'s is not even constant (the pause menu sets it from two angles). The default is the *angles*; the vector is derived. Shaders are **committed SPIR-V** so the build needs only a C compiler and SDL; `ctest shaders` fails on a stale blob. **Removed and worth recording:** bloom, DOF, haze, vignette and colour grade all shipped briefly and were cut — each touched every pixel and together they read as a filter over a screenshot (the bloom's bright pass was `BLENDMODE_MOD` of the frame over itself, which is squaring, not a threshold). Issue #63 redid the DOF grounded in real depth and it ships; **the bloom was measured and rejected** (C035, `tools/re/stage_lum.py`, I017). Over all 133 shipped background layers the art spans **0.000%** of pixels above 0.75 luminance (Brokeback Clif, whose brightest pixel is 0.541) to **22.602%** (Great Wall, whose sky is genuinely near-white art) — so no absolute threshold behaves as a bloom on both, and no relative one either, since gw's top-1% percentile is 1.000. Frame-side the same run: 766 px above 0.75 and **zero of them carrying a distance**, i.e. every bright pixel in a match frame is HUD, text or a sprite. Luminance is not a measured quantity here, it is a chosen one, which is the exact defect the first cut was for. A bloom would have to be an **authored emitter** in the `.stage` file beside the geometry — drawn again into the glow target, no threshold and nothing to tune — and that waits on #62 having content. **Still a POST-PROCESS over a finished picture**, reconstructing a normal from a silhouette gradient because that is all it has by the time it runs — the engine now has a stored depth buffer it has never been given. |
| Renderer: stage geometry | `runtime/video/mesh.c`, `runtime/video/mesh.h`, `runtime/video/stagegeom.c`, `runtime/video/stagegeom.h`, `tests/test_stagegeom.c`, `tests/test_stage_assets.c`, `runtime/shaders/mesh.vert`, `runtime/shaders/mesh.frag`, `docs/stage-geometry.md`, `stages/`, `tools/routes/stage_geom_test.sh`, `tools/routes/mesh_test.sh`, `tools/re/stage_gaps.py`, issues #49 #62 | **done** | **Hand-woven 3D sets, so HD2D has real depth to light.** `stages/<name>.stage` keyed on the stage's own `bg.dat` name, naming Wavefront OBJ models; `docs/stage-geometry.md` is the format. **`depth` is a property of the SOLID, not the vertex** — a pillar stands at one parallax depth while its vertices differ in x, jump and row — which is what lets a three-axis OBJ carry four-axis geometry. `depth: layer hill1.bmp` takes the depth from the stage's *own* layer (C031) so a solid cannot drift from the art it belongs with; a depth that cannot be derived is REFUSED, never defaulted. **The projection is the GAME'S, derived not chosen, and it is not a matrix**: a layer at depth *d* shifts by `camera/d` (C031), a fighter's z is used *directly* as a screen row (C018), but every object shifts by the camera **flat** whatever its z — two cameras glued together, which no perspective camera reproduces. `X - camera/d` is not linear in `(X, d, 1)`, so a 4x4 cannot express it either. Hence four independent per-vertex numbers, and the divide per vertex. `geom_stage_clip` is the copy under test; `mesh.vert` is a transcription and says so. **The record's own strings resolve the keys** (C033): `fn_0040c160`, the bg.dat parser, writes the stage's `name:` and each layer's bitmap path into the background record, so the port names both from the game's memory — no second decrypt, no `data.txt`, no load-order assumption. **`runtime/overrides/background.c` is where a set is placed**, because that override already holds the stage record, the layer spans, the view width and the draw-time camera. Placement is once per occupied gap in the layer order: a solid is drawn immediately before the first layer whose derived depth is `<= d`, an underivable layer depth reading as *infinitely far*. On the engine path each piece gets the **sliver** of depth between two list positions — ordered against the game's layers by the list, against other geometry in the same sliver by its own depth, which is exactly the pair that can interpenetrate. Measured: **1464 geometry draws inside the engine's pass, 0 separate mesh passes**, two a frame for two solids straddling the layers, 0 with no `.stage`. `mesh.c` is the standalone pass the SDL_Render path still needs, since it cannot take geometry at all; it keeps a self-test a broken depth test cannot survive (near triangle submitted FIRST, far one over it, plus a far-only pixel to rule out a pass that drew nothing). Most of `ctest stagegeom` asserts the loader **REFUSES**, because every failure mode of a data loader here is silent. **The PvE set pack is authored:** all twelve shipped Stage Mode backgrounds have original, low-poly solids in `stages/`; their documented layer anchors are checked by `ctest stage_assets` (the explicit HK Coliseum 1.0 fighters' plane is the only non-derivable exception). The guarded `stage_geom` route reaches the shared engine pass, with its no-geometry arm as the control. |
...
| HiDPI | `runtime/win32/win32.c`, `runtime/video/ddraw.c`, issue #56 | **done** | **Verified on a simulated 4K panel.** The window carries `SDL_WINDOW_HIGH_PIXEL_DENSITY`, the geometry is seeded from `SDL_GetWindowSizeInPixels` rather than the point size, and the pointer — which SDL delivers in POINTS — is multiplied by the density in `geom_pointer_to_compose`. `kwin_wayland --virtual --width 3840 --height 2160` with `kscreen-doctor output.1.scale.2` is a real scaled display: the port reports `794x550 points -> 1588x1100 pixels (display scale 2.00)` and composes 794 columns of world at scale 2.000 into the whole drawable. `tools/e2e.sh hidpi` is that run and REFUSES when the output comes up unscaled. `ctest geometry`'s `test_density` walks five densities OFFLINE and asserts the invariant across them — same field of view, world scale × density, the same window POINT on the same composition pixel — because a port composing from the point size has a self-consistent round trip at every single density and cannot detect its own error. Two simulations that do NOT work are recorded in the entry: Xvfb at any DPI (SDL's X11 backend reports points == pixels by construction) and kwin's own `--scale`, which is windowed-mode only. |
| Pause menu | `runtime/app/pause.c`, `runtime/overrides/screens.c`, issue #22 | **done** | Escape or Start during a match. Built on declining to call `fn_004246b0__orig`, which freezes the world with nothing to save or restore; the present lives inside that body, so it is issued separately, and the frozen frame is snapshotted because the menu draws straight onto the primary. **RESUME / DROP OUT / LEAVE MATCH / QUIT GAME**, on mouse, keyboard and pad. Escape is withheld from the game during a match so its own quit prompt does not open underneath. The rows are built per pause, because **DROP OUT is per player**: it appears only when the device that OPENED the menu is driving a slot this port put there (`coop_owns`), and it calls the same `coop_leave` that an unplugged pad does — so the menu records which device paused rather than guessing. **LEAVE MATCH** calls the game's OWN exit code directly — screens.c's `guest_end_match` and `guest_overlay_exit`, read off `fn_0041bc90` and `fn_00429730` — and **lands on the game's own front-end menu** (screen word `0x0044d020` = 10, `fn_00431d10`), which is what `tools/e2e.sh exit_to_menu` asserts by the WORD, because the two candidate screens share a blit destination and cannot be told apart from a frame dump. It used to overshoot to character selection, and the cause was the synthetic input it injected: `fn_00431c70` clears the held-button latch `fn_00431b70` edge-detects against on every way out of a match, so the second gather of a two-gather synthetic attack read as a FRESH press on the menu that had just appeared and confirmed whatever the cursor was on. That injection — F4 and the attack — is deleted (issue #22): the port invokes the exit, it does not fake the keystrokes. Both unpause FIRST `tools/e2e.sh exit_to_menu` asserts the landing by the screen WORD, because the two candidate screens share a blit destination and cannot be told apart from a frame dump. Both unpause FIRST — pausing works by declining to call the game's update, so a transition driven while frozen would be delivered to a game that never runs another frame. |
