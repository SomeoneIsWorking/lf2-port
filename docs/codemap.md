...
| Packaged Linux startup | `runtime/app/game_data.c`, `runtime/app/user_paths.c`, `runtime/ui/setup_ui.c`, `runtime/app/main.c`, Lucent `platform_c.h`, `tests/test_game_data.c`, `tests/test_config.c`, issues #115/#116 | **done** | Zero-argument startup validates the exact LF2 v2.0a executable and sibling `data/data.txt`, then enters its complete tree before guest loading. Discovery checks a developer path, persisted root, `game/` beside the outer `$APPIMAGE`, and source-tree layouts in that order. A missing/refused tree opens an SDL Browse/Quit setup flow and persists the valid root through Lucent's cross-platform OS user-data resolver; `--select-game` is the supported reset. A future Android shell must supply Lucent an Activity-owned app-private directory; SAF URI selection and durable permission remain Android release prerequisites. |
| AppImage release | `CMakeLists.txt` install rules, `platforms/freedesktop/`, `tools/build/appimage.py`, `tools/build/release_dependencies.py`, `.github/workflows/release-appimage.yml`, `tests/test_appimage.py`, issue #115 | **done** | The CMake install tree is the only packaging input. Pinned linuxdeploy/appimagetool/runtime hashes create an x86_64 artifact; both the staged tree and extracted final image are scanned to refuse original PE/game-data paths. Published releases build pinned SDL revisions on Ubuntu 22.04, run the default-target smoke and complete CTest suite, upload a workflow artifact, then attach the AppImage. The port-owned translated executable ships; the installer, original `lf2.exe`, and extracted assets do not. |
