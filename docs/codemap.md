...
| Global UI, cheats, input mapping and device art | `runtime/ui/settings_ui.cpp`, `runtime/ui/rmlui_input.cpp`, `runtime/ui/rmlui_backend.cpp`, `runtime/ui/device_assets.c`, `runtime/ui/device_icons.c`, `runtime/input/bindings.c`, `runtime/input/keyboard.c`, `runtime/input/gamepad.c`, `runtime/app/pause.c`, `runtime/app/function_keys.c`, `runtime/overrides/cheats.c`, `tools/routes/settings_test.py`, `tools/routes/ui_escape_test.py`, `tools/routes/ui_global_test.py`, `tools/routes/cheats_test.py`, issues #70 #79 #94 #104 #105 | **done** | Escape or Start opens one Dusklight-structured RmlUi shell on all four screens. Device-independent mapped input owns navigation; pointer coordinates are transformed once; shared SVG device art is embedded. The CHEATS pane is generated from one typed table containing only LF2.NET and F6-F9. Native leaves retain LF2's event/counter/world behavior while removing its hidden-code/mode and F3-lock gates; a bounded key-pulse queue guarantees release. During a match the document snapshots and pins LF2's three pause-pipeline words, freezing world updates while normal drawing and RmlUi continue. `cheats` proves LF2 consumes one F6 request; `ui_global` proves freeze and resume. |
