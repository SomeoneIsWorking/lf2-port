...
| Ultrawide backdrop coverage | `runtime/video/backdrop.h`, `runtime/video/ddraw.c`, `tests/test_backdrop.c`, issue #87, claim C045 | **fixed** | The widened far backdrop ended at y=198; beyond the original view, authored gaps between later keyed forest pieces therefore exposed black backing below it. Only the semantically marked far backdrop clamps its final source row behind those pieces, through both software and native paths, stopping at the measured world/caption boundary y=528. The reproduced Lion Forest gap has 0/2436 black pixels; native-width background output remains byte-identical to the recompiled body. |
