...
| Packaged startup and game selection | `runtime/app/game_data.c`, `runtime/app/game_selection.cpp`, `runtime/app/installer_extract.cpp`, `runtime/app/user_paths.c`, `runtime/ui/setup_ui.c`, `runtime/app/main.c`, Lucent `platform`/`zip`, `tests/test_game_data.c`, `tests/test_game_selection.*`, issues #115/#116/#120/#121 | **done on desktop; Android device acceptance pending** | Zero-argument startup validates the exact LF2 v2.0a executable and every file named by `data/data.txt`, then enters its complete tree before guest loading. Discovery checks a developer path, persisted root, `game/` beside the outer `$APPIMAGE`, and source-tree layouts. A missing/refused tree opens Browse/Quit; the shared selection owner accepts the original installer, an EXE, directory, or ZIP with exactly one `lf2.exe` at any depth. Installer and ZIP inputs are title-validated in a separate preparation tree before atomically replacing the accepted import, so a failed replacement preserves the working tree. The validated root is persisted through Lucent's OS user-data resolver; `--select-game` resets it. Android keeps native installer/ZIP preparation inside Activity-owned private staging until Java atomically commits the validated tree. |
| AppImage release | `CMakeLists.txt` install rules, `platforms/freedesktop/`, `tools/build/appimage.py`, `tools/build/release_dependencies.py`, `tests/test_appimage.py`, issues #115/#117/#121 | **done** | The CMake install tree is the only packaging input. Pinned linuxdeploy/appimagetool/runtime hashes create an x86_64 artifact with the scalable LF2 SVG icon; first run accepts the original installer, exact EXE, or a nested ZIP. Both the staged tree and extracted final image are scanned to refuse original PE/game-data paths. Maintainers build and gate the ignored artifact locally in Ubuntu 22.04, then upload it manually; no hosted workflow or generated package/source output is committed. |
| Android package shell | `platforms/android/`, `runtime/platform/android_bridge.c`, `runtime/input/touch_*`, `runtime/ui/touch_controls.c`, `runtime/audio/music_decode_libav.c`, `tools/build/android.py`, `tests/test_android.py`, issues #120/#121 | **wip: native ARM64 verified; signed/device release gates pending** | SDLActivity and SAF copy a selected original installer, complete folder, or ZIP into bounded app-private staging; native setup prepares installer/ZIP contents inside that wrapper, validates the complete tree, and Java atomically commits only that validated directory. Lucent owns neutral touch routing and ZIP extraction; the port owns title-specific installer decoding, the safe-area-aware LF2 layout, shared action ledger, controller suppression, cancellation, and overlay. Android links pinned bzip2 for installer streams and decodes WMA in process through pinned static FFmpeg. The NDK 28 ARM64 `libmain.so` links with NEON and exports `main`; release assembly refuses missing signing, checks APK contents, and verifies signatures. A supported JDK, real signing identity, one connected device, installed setup/touch/audio/lifecycle checks, and the named-device sustained performance matrix still block verified publication. |
