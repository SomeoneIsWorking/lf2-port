...
| Fresh-machine provisioning | `bootstrap.py` (stdlib-only), thin `run.sh` shim, `pyproject.toml` + `uv.lock`, `tests/test_bootstrap.py` | **done** | `./run.sh` execs `bootstrap.py`, which validates explicit port-assets, reuses/provisions `SHARED_DIR`, reuses a complete established shared checkout, or clones a pinned revision into gitignored `scratch/deps`; it requires both consumed SVGs and passes the resolved path explicitly to CMake. It initializes RmlUi, runs `uv sync`, extracts the game tree only when absent from `$LF2_INSTALLER`, a local copy, or lf2.net, rebuilds when any shipping input is newer (`REBUILD=1` forces), then starts from `game/`. Every missing prerequisite refuses BY NAME with its fix; nothing is fetched silently |
...
| Direct startup | `runtime/app/startup.c`, `runtime/overrides/boot_guest.c`, `runtime/overrides/text.c`, `runtime/win32/win32.c`, issues #71 #89 #102 | **done** | The world constructor is overridden at `fn_00419e40`: the complete original constructor runs, then marks the required one-time load before the first update accepts input. The update boundary routes only exact retired top mode 0 to that same load marker; unknown values remain visible rather than masking corruption. One synchronous startup call first lets the mode-0 shadow body perform its one-time `DAT_0044d068` keyboard-table/menu-resource initialization while drawing and presentation are declined; this keeps the guest's mappings and constructors authoritative without running an interactive front end. It then bypasses mode 1—the loading-picture setup/present branch—and calls the real mode-2 `fn_0041bc90` initializer once with its exact guest ABI. Progress clip draws and presents are declined at their guest-owned boundaries only during that call. SDL is visible from creation; no renderer frame gate or delayed `ShowWindow` exists, and ordinary mode-menu presentation begins after the blocking call returns. The launcher, its controls editor, replay and legacy network branches are unreachable together. |
| Global UI, input mapping and device art | `runtime/ui/settings_ui.cpp`, `runtime/ui/rmlui_input.cpp`, `runtime/ui/rmlui_backend.cpp`, `runtime/ui/device_assets.c`, `runtime/ui/device_icons.c`, `runtime/ui/overlay_panel.c`, `runtime/ui/ui_rgba.c`, `runtime/ui/ui_rgba.h`, `runtime/input/bindings.c`, `runtime/input/keyboard.c`, `runtime/input/gamepad.c`, `runtime/app/pause.c`, `runtime/app/config.c`, `tests/test_bindings.c`, `tests/test_device_icons.c`, `tests/test_overlay_panel.c`, `tests/test_overlay_font.py`, `tests/test_ui_rgba.c`, `tools/routes/settings_test.py`, `tools/routes/ui_escape_test.py`, `tools/routes/ui_global_test.py`, issues #70 #74 #77 #79 #80 #82 #84 #88 #94 | **done** | Escape or Start opens one Dusklight-structured RmlUi shell directly on mode menu, character selection, overlay, and match. `rmlui_input.cpp` is the Dusklight-style device-independent boundary: configured keyboard and every attached controller feed Up/Down/Left/Right/Confirm/Cancel with latched edges and directional repeat. The physical X11/XTEST route proves Escape, mapped keyboard Confirm, and a real mouse click; the settings route proves mapped controller focus/Confirm reaches Controls. Pointer events use SDL's renderer transform once—not two density multiplies—and the context uses drawable dimensions plus content scale, so its outline font is 32px on the verified 2x display. While active RmlUi consumes all physical input before Win32 messages and zeros the guest gather. Shared `port-assets` SVGs are embedded rather than copied; their keyboard is a low, wide key field and their controller has a distinct ergonomic silhouette, both verified at the 18px shipping size and the 48px HUD size. Native icons use the same linear host-coverage sampling as fonts and RmlUi uses a 120x120 linear-filtered raster. `ui_rgba.c` is the authoritative straight-ARGB premultiplication and opaque-composition seam shared by device icons and the pre-fight panel; `test_ui_rgba.c` exercises that shipping implementation. In-match indicators remain absent while the pre-fight panel borrows the HUD signal. Character-select indicators are recorded before that panel's own blit rather than appended at present, so the panel covers them by ordinary painter order. `overlay_panel.c` owns the port-authored pre-fight panel layout and its exact-output outline raster; `runtime/overrides/text.c` only identifies FUN_00429730's producer slot. The panel shares the decompiled input-row geometry, keeps the game's later dynamic value draws, and embeds a licensed 17-outline CJK subset whose test derives coverage from the runtime literals. `ui_global` now checks exact 1920x1080 modal open/close pixels. |
...
| Match modal behavior | `runtime/app/pause.c`, `runtime/ui/rmlui_lifecycle.h`, issues #22 #94 | **done** | The global RmlUi shell is modal through input ownership, not through a second render lifecycle: `fn_004246b0__orig` keeps the game on its ordinary update/draw/present path behind the document, and RmlUi composites after that current frame. Snapshot capture/restore, retained software regions, on-demand frozen presents, and spent/deferred list clearing are deleted. A document-generation token cancels the UI frame if mapped input closes or replaces the document from inside `rmlui_render`, before backend rendering can continue on a hidden document. **DROP OUT** appears only when the opening device owns a port-added fighter and calls the same `coop_leave` as disconnect. **LEAVE MATCH** sends one F4 pulse through the normal game path. `ui_global` runs a deterministic no-match-modal control at identical offsets: outside-document pixels and the first hidden frame (+362) plus two successors must match exactly. `pause_dropout` verifies per-device drop, and `leave_match` verifies the game's result-overlay receipt. |
...
| Rendering | `runtime/video/ddraw.c`, `runtime/win32/gdi.c`, `runtime/ui/device_icons.c`, `runtime/ui/overlay_panel.c`, `runtime/ui/ui_rgba.c`, `tools/routes/overlay_labels_test.py` | **done** | Menus, screens, colour-keyed sprites, device SVGs, and game/GDI text render. Non-image text uses required embedded outline faces and output-resolution host tiles; the shipping-engine HiDPI route verifies 2x game glyph rasterization. `overlay_panel.c` covers the pre-fight labels baked into original screen art by appending a complete native panel after FUN_00429730's exact final static producer and before its dynamic values. Every original CHARMENU draw stays underneath as the GPU-resource fallback. The panel is redrawn at logical composition size with black value wells derived from the dynamic-value anchor and input rows, then separately rasterized into an exact-output host tile; sprites and unrelated bitmap art remain pixel-scaled. `overlay_labels` authenticates both 3840×1975 mode branches, the 1092×596 raster, dynamic values, wells, all 20 CJK glyph cells and every Latin run, within-logical-texel edge detail that nearest-upscaled and native-CJK-only hybrid negatives lack, zero dropped resources, and a forced native-failure capture that exposes the retained authored bitmap. |
...
| Menus: one input model | `runtime/overrides/menu.c`, `runtime/overrides/screens.c`, `runtime/ui/settings_ui.cpp`, `runtime/ui/rmlui_input.cpp`, `runtime/app/pause.c`, `tools/routes/mouse_test.sh`, `tools/routes/settings_test.py`, `tools/routes/ui_escape_test.py`, `tools/routes/ui_global_test.py` | **done** | Normal boot starts at the post-load **mode menu**. Mode selection, character selection, and the pre-fight overlay retain the game's own behavior. Escape/Start opens RmlUi directly everywhere; no legacy pause screen exists. A mouse-only route reaches a match, settings drives RmlUi by mapped controller actions, and the physical-input route drives it by XTEST keyboard and mouse; `ui_global` opens the same shell in all four contexts. The retired launcher has no reachable replay/network choices. |
