...
| Drop-in coop (RE) | `runtime/overrides.c`, issues #15 #16 | **wip** | The blocking RE is done and a fighter can be added to a **running** match, built the way the game builds one. `this` is `0x00458b00`; `this+404` is **400** object pointers on a `0x420` stride (the player slots are its first eight, all pre-allocated and non-null always); an object is in the world iff the **byte at `0x00458b04 + index`** is 1, the test `fn_004064d0` makes as it walks the table; `this+2004` points at the **object-data registry**, whose blocks carry the `data.txt` **object id at +1780** (checked against the game's own `data.txt`: all 65 entries match, and the only two absent ids are backgrounds); `fn_004061d0` is the record's `__thiscall` reset and `+0x368`/`+872` its data pointer. `LF2_COOP_SPAWN=<index>[,<id>]` resets a free entry, points it at the registry block for that id, copies a position and sets the gate — putting a **Bandit** into a match of two other characters. The spawned fighter is complete enough to get the **right HUD portrait** — measured two-sided inside single runs, since VS randomises. What remains is the feature: `+0x354` has no established meaning, the free-index rule is unknown (the game gave its computer opponent 11 with 1..10 free), and the gather still caps at `i < 4` |
