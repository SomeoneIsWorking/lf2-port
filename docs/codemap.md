...
| Native renderer | `runtime/render.c`, `runtime/render.h`, `runtime/hd2d.c`, `runtime/shaders/`, `tools/render_test.sh`, `tools/build_shaders.sh`, issue #30 | **wip** | The game's draws as **GPU geometry**. `ddraw.c` records a display list per destination surface; the game composes off-screen and copies to the primary, so the source of that copy *names* the composition and the frame boundary is discovered from the game's own blits with no address hardcoded. **The colour key becomes alpha** on upload — the port's first blend stage, the thing claim C010 said made shadows impossible. Drawn into a **render target at the window's resolution**, not the game's, `SDL_SCALEMODE_NEAREST` throughout: the software path fills a 794x550 buffer and point-scales it, so on a 1080p screen every pass ran on an upscaled thumbnail and every sprite was resampled twice. GDI text, which never goes through `Blt`, rides as premultiplied **tiles** at their real place in the list. The renderer is created as SDL's **`gpu`** backend by name, not by SDL's default order — the OpenGL backend it would otherwise pick has no `SDL_GPUDevice`, hence no `SDL_GPURenderState` and no shaders at all. **Isometric lighting and sprite-cast shadows** (`hd2d.c`) run in real fragment shaders: one key light as a direction in the stage's own axes (x across, y up = LF2's jump axis C018, z toward the camera), a hemisphere ambient, and a bevel normal built from the gradient of the sprite's silhouette — applied **only to the objects standing in the stage**, which the game identifies for free by drawing a shadow ellipse at their feet immediately before them (C019). The scenery, HUD and text come through as the exact pixels the game composed; the only thing that reaches the floor is the shadow. That shadow is the sprite's own silhouette sheared along the **same** light vector through `SDL_RenderGeometry` (`RenderTexture` cannot shear), into a mask that needs its own shader because SDL's fixed-function multiply gives `sprite.rgb * a` — the fighter's colours — and made a shadow darker under the bright parts of them. Shaders are **committed SPIR-V** (`tools/build_shaders.sh`, `ctest shaders` fails on a stale blob), so the build still needs only a C compiler and SDL; a backend that cannot take SPIR-V says so and gets the plain composition, with no approximation to fall back on. `ctest render` runs four arms: the GPU frame matches the software compositor to **max 1–2 levels of 255** (antialiased glyph edges only), dropping every 7th draw must differ, the light must change the **match** frame, and the light must change **nothing** on the menu frame — that last one is what catches an effect spreading over the whole picture. **Removed, and worth recording:** a bloom, depth of field, atmospheric haze, vignette and colour grade all shipped here briefly and were cut — each touched every pixel and together they read as a filter over a screenshot rather than light in a scene. The bloom's bright pass was `SDL_BLENDMODE_MOD` of the frame over itself, which is squaring, not a threshold, so the whole frame contributed to the glow. **Not done:** the stage itself is still flat painted layers (there is no depth in the data beyond the parallax rate, which already *is* the projection), the floor is lit as a billboard rather than as a ground plane, and GDI text is still rendered at the composition's resolution rather than the output's. The pause menu and controls hint still present through the software path because they draw straight onto the primary and are in no display list |
