...
| Android package shell | `platforms/android/`, `runtime/platform/android_bridge.c`, `runtime/platform/window_policy.c`, `runtime/app/update.c`, `runtime/input/touch_*`, `runtime/ui/touch_controls.c`, `runtime/audio/music_decode_libav.c`, `tools/build/android.py`, `tests/test_android.py`, issues #120/#121/#122/#123/#124 | **wip: native ARM64 and debug package verified; signed/device release gates pending** | SDLActivity and SAF import the original installer, complete folder, or ZIP through bounded private staging. Before SDL creates its window, the port limits SDLActivity to both landscape orientations; non-control touch contacts use the same window-to-guest pointer route as a mouse while authored action zones retain multi-touch ownership. The updater checks GitHub Releases on startup and from Settings, accepts only a newer same-package APK with the installed signing certificate, then hands it to Android's user-confirmed installer. Semantic versions deterministically own increasing version codes. The NDK 28 ARM64 library and Gradle 9.7.1/AGP 9.3.0 debug package build and pass package inspection; a real signing identity and device setup/touch/orientation/audio/lifecycle/performance acceptance still block verified publication. |
