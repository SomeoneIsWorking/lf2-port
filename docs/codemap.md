...
| Renderer: engine and full-resolution output | `runtime/video/engine.c`, `runtime/video/engine_textures.c`, `runtime/video/texture_lru.h`, `runtime/video/engine.h`, `runtime/video/render.c`, `runtime/video/texrect.h`, `tests/test_texture_lru.c`, `tests/test_texrect.c`, `tools/routes/fullres_test.py`, `tools/routes/render_test.sh`, `tools/routes/texture_cache_test.py`, issues #41 #64 #76 #83 #87 | **partial** | One SDL_GPU device, painter-order D32 depth, three blend pipelines, and a frame-safe texture cache. The display list is unchanged; the engine replaces what draws it. Composition dimensions are world/FOV coordinates only: `render.c` transforms every sprite, mesh, fill and host tile into output pixels, and `engine_draw` rasterises directly into a drawable-sized target that is copied 1:1. Guest sprites remain nearest; full-output host font/SVG coverage is linear. `tools/e2e.py fullres` is the persistent 3840x1975 falsifier: it requires the engine's own 3840x1975 target report, a 1070x550 field of view whose ceiled extent covers the output, an exact-size captured frame, and zero black pixels in either outer column; its PPM edge reader rejects a synthetic black band before trusting the capture. `texrect.h` owns source sampling, including DirectDraw's stretched-rectangle map. The cache protects every texture referenced by the current frame and the Stage route previously filled all 512 entries with no lost art. **Gap:** issue #76 is reopened because its old whole-scene upscale was not a valid jitter fix; fractional-scale scrolling needs a per-draw sampling verification/fix. The software renderer remains the flattened-frame A/B control. |
