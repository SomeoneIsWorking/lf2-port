...
| Direct startup | `runtime/app/startup.c`, `runtime/overrides/boot_guest.c`, `runtime/video/ddraw.c`, issue #71 #89 | **done** | The world constructor is overridden at `fn_00419e40`: the complete original constructor runs, then its initial top-level state is set to the local loader before the first update can draw or accept input. The update boundary also routes the exact retired launcher mode 0 through that loader before the original body can dispatch it; unknown values remain visible rather than masking unrelated corruption. This is a product-state invariant, not a hidden blit filter: the launcher's sub-screen 6 controls editor and every sibling launcher branch are unreachable together. The loader frames are discarded while the SDL window is hidden; the window is revealed only after the first mode-menu frame has been rendered and presented. The first script-visible screen is `modemenu` at frame 4. `wsock.c` is only a loud binary-compatibility stub, not the port's future multiplayer design. |
...
and character-mask passes. The mask reloads the scene's completed D32 target and accepts a
