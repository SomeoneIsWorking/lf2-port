...
| Widescreen | `runtime/ddraw.c`, `runtime/win32.c`, `runtime/overrides.c` | **done** | `LF2_WIDESCREEN=<w>[x<h>]` gives a genuinely wider field of view, not a scaled picture: the compose surface widens and the game's own three viewport-width words (`0x0044d014`, `0x0044d78c`, `0x00453cd4`) are set to match, so the camera and the layer loops draw MORE WORLD. Everything that is fixed at 794 is centred instead — front end, mode menu, character select, overlay — with the pointer shifted the other way so hit tests still line up. Full-width colour fills and single-blit backdrops are carried across the viewport; the HUD strip's 198x54 panels are tiled out to the edge |
