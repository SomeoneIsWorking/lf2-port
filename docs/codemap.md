...
| Renderer: object-sprite sampling chain | `runtime/video/spritefilter.h`, `runtime/shaders/quad.frag`, `runtime/video/engine.c`, `runtime/app/options.c`, `runtime/ui/settings_ui.cpp`, `tests/test_spritefilter.c`, `tools/routes/sprite_passes_test.py`, issue #112 | **done** | A player builds an ordered chain of resampling passes over magnified object sprites, plus edge smoothing and a silhouette outline, from the GRAPHICS tab: `nearest`/`linear` passes with rational or AUTO factors, `aa`, `outline:1..2`. `spritefilter.h` owns what a chain may hold, what AUTO resolves to per quad, when a quad must draw alone (its taps clamp into its own uv rect, and sheet frames butt edge to edge) and how far the geometry grows to make room for an outline; `quad.frag` evaluates the chain analytically -- no intermediate render targets, at most one LINEAR cut so the committed SPIR-V stays 38 KB rather than 174 KB. A spec that does not parse is refused by name and leaves the chain empty. The config key is `sprite_passes`; `LF2_SPRITE_PASSES` is the route pin. `tools/e2e.py sprite_passes` gates it on a real match frame: `nearest:2` must be BYTE-IDENTICAL to no chain (it was 2,084 pixels wrong while taps were addressed from the frame's uv origin rather than a whole sheet texel), while `nearest:1/2` and `outline:1` must both change the frame and the outline must paint near-black pixels. `ctest spritefilter` walks the parser, the caps, the AUTO factor, the outline margin and the coordinate walk in a millisecond. |
