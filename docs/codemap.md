...
| Packaged Linux startup | `runtime/app/game_data.c`, `runtime/app/user_paths.c`, `runtime/ui/setup_ui.c`, `runtime/app/main.c`, `tests/test_game_data.c`, `tests/test_config.c`, issues #115/#116 | **done** | Zero-argument startup validates the exact LF2 v2.0a executable and sibling `data/data.txt`, then enters its complete tree before guest loading. Discovery checks a developer path, persisted root, `game/` beside the outer `$APPIMAGE`, and source-tree layouts in that order. A missing/refused tree opens an SDL Browse/Quit setup flow and persists the valid root under XDG/SDL per-user configuration; `--select-game` is the supported reset. Android does not reuse this POSIX path: Activity/SAF URI persistence remains a release prerequisite. |
