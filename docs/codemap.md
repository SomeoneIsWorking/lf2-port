...
| Drop-in coop (RE) | `runtime/overrides.c`, issue #15 | **wip** | The blocking RE question is answered and demonstrated: `this` is `0x00458b00`, `this+404` is **400** object pointers on a `0x420` stride (the eight player slots are its first eight entries, all pre-allocated and non-null at all times), and an object is in the world iff the **byte at `0x00458b04 + index`** is 1 — the test `fn_004064d0` makes as it walks the table. `LF2_COOP_SPAWN=<index>` clones a fighter into a **running** match: it animates, walks, collides, takes damage and draws with its own HUD bar, where the identical clone without the gate byte is inert for 300 frames. What remains is the feature, not the mechanism: the game's own spawn is mapped in issue #16 — `fn_004061d0` is the record's reset, `this+2004` the object-data registry, `+0x368` the data pointer — so the clone probe can be replaced by it |
