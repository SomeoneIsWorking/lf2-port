...
| Global RmlUi, input mapping and device art | `runtime/ui/settings_ui.cpp`, `runtime/ui/rmlui_backend.cpp`, `runtime/ui/device_assets.c`, `runtime/ui/device_icons.c`, `runtime/input/bindings.c`, `runtime/input/gamepad.c`, `runtime/app/pause.c`, `runtime/app/config.c`, `tests/test_bindings.c`, `tools/routes/settings_test.sh`, `tools/routes/ui_global_test.sh`, issues #70 #77 #79 #80 | **done** | Escape or Start opens one Dusklight-structured RmlUi shell directly on mode menu, character selection, overlay, and match; the removed hand-painted pause menu is no longer a prerequisite. RmlUi owns presentation/navigation and its SDL backend remains a separate adapter. While active it consumes all physical input before both Win32 messages and the native input gather; only a match freezes, and its retained native display list is rewound under the modal. Keyboard/controller mappings for all seven actions persist and feed the shipping gather. Shared `port-assets` SVGs are embedded rather than copied. The existing high-resolution native quad path rasterizes each SVG to its current output-pixel footprint behind the unchanged 18x18 logical icon and refreshes it on resize; RmlUi receives a 120x120 linear-filtered raster, and software retains its logical raster. The `settings` and `ui_global` routes prove document rendering, both SVG textures, mapped navigation, and all four screen contexts. |
...
| Input path | `runtime/win32/win32.c`, `runtime/input/bindings.c`, `runtime/overrides/input.c`, `runtime/ui/settings_ui.cpp` | **done** | Keyboard, mouse and controller enter game state through their native paths; RmlUi edits the action maps consumed by those shipping paths. While the global document is open it consumes physical SDL input, withholds Escape from the guest, and zeros the direct keyboard/pad gather so no modal action leaks into the underlying game. |
