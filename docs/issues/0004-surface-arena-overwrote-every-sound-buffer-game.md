...
Fix: runtime/guest_map.h declares every arena base/size in one place, _Static_assert fails the build on overlap, and both allocators abort at their reservation end. Map uses #define not enum -- 0x90000000 as an enum constant is a negative int and bounds checks against it silently pass.

Ruled out along the way (do not re-derive): mixer resampling (unit-tested, runtime/test_mixer.c, with the old implementation as a control -- it already reproduced pitch correctly at every rate); WAVEFORMATEX parse (matches file headers; odd rates 10989/11300/20200/22100/23600/38400/44000 are genuinely in the files); MAX_BUFS overflow (0 unregistered); mixer under-delivery (max request 512 vs 4096 slice, never fires); music use-after-free (real, fixed separately, but not this symptom).
