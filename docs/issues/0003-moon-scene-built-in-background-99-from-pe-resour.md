...
**The one real defect:** the moon (BACK99_2) draws as an opaque black box -- its black background should be transparent. h_StretchBlt ignores its rop argument (runtime/win32/gdi.c), so if the game uses a SRCAND/SRCPAINT pair for this draw the port copies opaquely; alternatively the surface misses its SetColorKey. Needs a live bg-99 repro to trace which; background selection is random, so build a deterministic repro first (find the bg-index variable, or add a diagnostic override).
