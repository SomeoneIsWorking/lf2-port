...
Root cause was the %lf scanf store: gscan dropped the l modifier and stored 4 of 8 bytes, so every parsed double (all character physics: walking_speed, jump_height, ...) kept its magnitude from stale guest heap residue -- sometimes plausible, sometimes zero, which is a world where nobody can move. Fixed in runtime/win32/imports.c (scan_store honours is_long). 8/8 freeze probes live after the fix, vs ~50-75% frozen before. Same root cause as the Mac walk-in-place/jump-softlock reports.
