...
The native-resolution game text path never disappeared; RmlUi had introduced a separate outline-font atlas and incorrectly sized dp layout from SDL_GetWindowPixelDensity instead of the display content scale. RmlUi now keeps drawable-sized geometry, resolves scale through SDL_GetWindowDisplayScale so FreeType rasterizes at the actual UI pixel size, and linearly samples generated coverage textures. The simulated 4K/200% run reports a 1588x1100 drawable, content scale 2.00, and a 32px raster for the 16dp body font.
