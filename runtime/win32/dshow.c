 ...
 * and IMediaPosition. WMA is decoded by whatever ffmpeg is on PATH (see runtime/audio/dsound.c),
...
static const uint8_t N_MPOSITION[] = {
    3, 1, 1, 2, 4, 6, 9, 2, 3, 2, 2, 3, 2, 3, 3, 2, 2, 2,
};
static const uint8_t N_BAUDIO[] = {
    3, 1, 1, 2, 4, 6, 9, 2, 2, 2, 2,
};
static const uint8_t N_MEVENT[] = {
    3, 1, 1, 2, 4, 6, 9, 2, 5, 3, 2, 2, 4, 4, 2, 2,
};
static const uint8_t N_MSEEK[] = {
    3, 1, 1, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 5, 5, 3, 3, 3, 2, 2,
};
...
static void note_no_music(void)
{
    static int said;
    if (!said++)
        lf2_log_writef(LF2_LOG_INFO, "dshow",
                       "note: could not read the music filename from the graph -- "
                       "background music will be silent\n");
}

static void ds_ok(uint32_t self)
{
    (void)self;
    com_ret(cur_nargs(), DD_OK);
}
...
/* IMediaControl */
static void mc_Run(uint32_t self)
{
    (void)self;
    music_start();
    graph_running = 1;
...
static void mc_Stop(uint32_t self)
{
    (void)self;
    music_stop();
    graph_running = 0;
...
static void mc_Pause(uint32_t self)
{
    (void)self;
    music_stop();
    graph_running = 0;
...
static void ba_put_Volume(uint32_t self)
{
    ...
    if (lf2_environment_get(LF2_ENV_AUDIO_DEBUG)) {
        static int n;
        if (n++ < 3) lf2_log_writef(LF2_LOG_INFO, "dshow", "put_Volume(%d) centibels\n", (int32_t)ARG(1));
    }
...
static void render_file(uint32_t bstr)
{
    char path[512];
    if (lf2_environment_get(LF2_ENV_AUDIO_DEBUG)) {
        lf2_log_writef(LF2_LOG_INFO, "dshow", "RenderFile bstr=%08x len=%u first16=", bstr, bstr ? LD32(bstr - 4) : 0u);
        for (int i = 0; bstr && i < 16; i++) lf2_log_writef(LF2_LOG_INFO, "dshow", "%02x ", LD8(bstr + i));
        lf2_log_writef(LF2_LOG_INFO, "dshow", "\n");
    }
    if (!bstr_to_path(bstr, path, sizeof path)) {
        note_no_music();
        return;
    }
    ...
    lf2_log_writef(LF2_LOG_INFO, "dshow", "music: loaded %s\n", path);
}

static void mc_RenderFile(uint32_t self)
{
    (void)self;
    render_file(ARG(1));
    com_ret(2, DD_OK);
...
static void gb_RenderFile(uint32_t self)
{
    (void)self;
    render_file(ARG(1));
    com_ret(3, DD_OK);
